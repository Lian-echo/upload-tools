...
// 实现自动化的底层原理，真正使用只需要安装插件upload-tools，移步README.md
type Config = {
  host: string;
  username: string;
  password: string;
  ...
  try {
    ...
    await ssh.putDirectory(localPath, remotePath, {
      ...
      validate: (itemPath: string) => {
        ...
        return baseName.substr(0, 1) !== "." && baseName !== "node_modules" && baseName !== ".git";
      },
    });
...
async function uploadTools({ commands, config }: { commands: CommandsType; config: Config }) {
